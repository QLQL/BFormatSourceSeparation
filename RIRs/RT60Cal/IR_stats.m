function [rt60,drr,cte,cfs,edt] = IR_stats(filename,varargin)
...
for n = 1:numchans
    ...
    for f = 1:length(cfs)
        ...
        z(f,:,n) = temp(end:-1:1);
        [rt(f,n),E_rt,fit_rt] = calc_decay(z(f,t0:end,n),options.y_fit,60,fs); % estimate RT
        ...
        if options.graph % plot
            ...
            if nargout==5
                legend('Energy decay curve',['Linear fit (RT' num2str(abs(diff(options.y_fit))) ')'],'Linear fit (EDT)','location','northeast')
            else
