...
%     obj = histogram(args{:},'Parent',cax);
    obj = histogram(args{[1,4]},'Parent',cax,'FaceColor',[1,1,1]*0.5,'LineStyle','None');
    hold on; obj = histogram(args{[2,4]},'Parent',cax,'FaceColor',args{5}(1,:),'LineStyle','None');
    hold on; obj = histogram(args{[3,4]},'Parent',cax,'FaceColor',args{5}(2,:),'LineStyle','None');
    
    set(cax,'ThetaAxisUnits','radians');
    set(cax,'ThetaTickLabel',{'0','\pi/6','\pi/3','\pi/2','2\pi/3','5\pi/6','\pi','-5\pi/6','-2\pi/3','-\pi/2','-\pi/3','-\pi/6'})
    set(cax,'RTickLabel',[]);
