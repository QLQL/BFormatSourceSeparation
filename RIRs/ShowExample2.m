...
sample_i = 120;
source_i = 2; % show the first signal
SourceNumber = 2;
...
taglegArray = {{'Oracle IBM','IBM'},
    ...
    {'Chimera',sprintf('ChimeraNsrc%dWei',SourceNumber)},
    {'Chen etal','GMMNew'}
    };
...
axisrange = [-25,5];
figure('position',[1200,100,500,800]);
...
text(20,450,'Groundtruth','FontSize',12);
...
hold on
plot(temp1,temp2,'k','linewidth',2)
text(20,450,'Mixture','FontSize',12);
...
for algorithm = 1:6,
    ...
    caxis(axisrange)
    text(10,400,legArray{algorithm},'FontSize',12);
    set(gca,'LineWidth',2,'Layer','top')
    ax2 = axes('Position', get(h, 'Position'),'Color','none');
    set(ax2,'LineWidth',2,'XTick',[],'YTick',[],'XColor','w','YColor','w','box','on','layer','top')
