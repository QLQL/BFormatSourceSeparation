...
for sample_i in range(N): # N
    ...
    for n, i in enumerate(chooseIndex):
        ...
        for source_i in range(args.srcNum):
            ...
            # Use either the IBM from the DC head or half-IBM-half-IRM from both heads
            if args.featureFlag == 2:
                temp = mixLP[n] + 20 * np.log10(np.maximum(tempMask, 0.001)) / hparams.norm_scale_db
            else:
                half_mask = tempMask.copy()
                irm_mask = estimate_irm[n, :, :, source_i]
                half_mask[tempMask == 0] = irm_mask[tempMask == 0]
                temp = mixLP[n] + 20 * np.log10(np.maximum(half_mask, 0.001)) / hparams.norm_scale_db
    ...
    for source_i in range(args.srcNum):
        ...
        dst_wav_path = "{}/Ind_{}_{}_est{}.wav".format(dst_dir, sample_i + 1, tag, source_i)
